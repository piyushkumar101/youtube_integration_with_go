...
func getAllPlaylists() []PlaylistItems {
	var url string = fmt.Sprintf("https://www.googleapis.com/youtube/v3/playlists?channelId=%s&key=%s&part=snippet&maxResults=50&type=playlist", channel_id, api_key)
	log.Printf("Visting : %s\n", url)
	response, err := http.Get(url)
	if err != nil {
		log.Println(err)
